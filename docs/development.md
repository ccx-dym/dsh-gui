...
| 项目 | 实测值 |
| --- | --- |
| RC 版本 / commit | `0.1.0` / `b1306f0`（另含本节未提交验收记录） |
| 安装包字节数 / SHA-256 | 3,623,760 / `D497E2DB7C589D9BD54A09D9868CBCB16165AE146023AFE95399E9A393D5D520` |
| Authenticode 状态 | `NotSigned`，仅限个人/小范围测试 |
| Windows 版本 / build | Windows 11 专业版 / 26200 |
| CPU / 逻辑处理器 / 内存 | AMD Ryzen 9 9950X3D / 32 / 95.6 GiB |
| WebView2 版本 | 151.0.4129.93 |
| DSH 版本 / 适配器状态 | 本机未配置兼容发布源，runtime 未启动；适配器真实页面验收未执行 |
...
| DSH 业务交互无回归 | 待实测 | 待记录 |
| 不支持版本/DOM 失败关闭回退 | 自动门禁通过 | 真实 DSH 页面因本机未配置发布源待实测 |
...
| DSH 双门 ready（ms） | N/A：未配置 runtime 发布源 | 待实测 | 真实 DSH 待发布源 |
...
单实例。阶段 2 已实现兼容 runtime 的安全下载、隔离 probe 与事务激活；Phase 3 已进入
沉浸式皮肤实现与验收阶段。真实安装器、桌面自动更新和皮肤性能仍必须以本轮 RC 构建及
上述手工矩阵的实际证据为准。
