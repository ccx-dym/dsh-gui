...
3. 逐一验证 cover、contain、stretch、center 和九个位置；确认页面始终只有一个背景合成层，
   模糊和图片不透明度只影响背景图片，浅/深遮罩保持独立，正文和控件仍可读。
...
8. 检查主窗口自绘标题栏：可拖动窗口，双击可切换最大化/还原，最小化、最大化和关闭
   按钮均可用；关闭仍遵循“最小化到托盘”的既有语义。
9. 启用沉浸式皮肤后，确认图片连续覆盖标题栏、左侧栏和对话区，中间没有不透明纯色
   断层；欢迎页和普通会话的输入卡片均透明，文字、图标和焦点状态仍清晰可见。
10. 未配置皮肤或恢复默认后，确认自绘标题栏仍可正常显示和操作；“外观设置”和“检查
